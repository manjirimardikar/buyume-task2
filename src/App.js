...
{"id":50,"first_name":"Melisenda","last_name":"Queree","email":"mqueree1d@intel.com","gender":"Female","age":52}
  ...
  const handleRemoveFromTeam = (index) => {
    const updateTeam = teamMembers.filter((_, i) => i!== index);
    setTeamMembers(updateTeam);
  };

  const calculateAverageAge = () => {
    const totalAge = teamMembers.reduce((sum, member) => sum + member.age, 0);
    return teamMembers.length > 0 ? totalAge / teamMembers.length : 0; 
  };
    ...
    <div className="App">
      ...
      <ul>
        {allEmployees.map((employee, index) =>(
          <li style={{border:'1px solid black', backgroundColor:'lightgrey', padding:'10px',margin:'10px'}} key={index}>
            {employee.first_name}    {employee.age}
            <button style={{margin:'10px', backgroundColor:'lightblue', border:'none'}} onClick={() =>handleAddToTeam(employee)}>Add</button>
          </li>
        ))}
      </ul>
      </div>
      <div className='box2'>
        <h2>Team</h2>
        <button style={{margin:'10px', backgroundColor:'pink', border:'none'}} onClick={handleSortByAge}>Sort by Age</button>
        <ul>
          {teamMembers.map((member,index) =>(
            <li key={index}>
              {member.first_name} {member.age}
              <button style={{margin:'10px', backgroundColor:'lightblue', border:'none'}} onClick={() => handleRemoveFromTeam(index)}>Remove</button>
            </li>
          ))}
        </ul>
    <p style={{border:'1px solid black', backgroundColor:'grey'}}>Average Age: {calculateAverageAge()}</p>
      </div>
    </div>
  );
}
